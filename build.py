...
def _fdc_bankroll_race_svg(book):
    ...
    # n points per line = the opening bankroll plus one point per settled market,
    # so every series visibly leaves the same $1,000 mark on the left edge.
    n = len(events) + 1
    def X(i):
    ...
    for key, name, avatar, criterion in FD_PERSONAS:
        ...
        vals, bal = [FD_BANKROLL_START], FD_BANKROLL_START
        for ev in events:
            b = by_event.get(ev)
            if b is not None:
                bal += b.get("pnl", 0.0)
    ...
    # Label the baseline on the LEFT: every line leaves the $1,000 mark there and
    # only falls at first, so the space above it is clear — the right edge belongs
    # to the end-of-race monogram chips.
    grid.append(f'<text x="{X0 + 8}" y="{base_y - 8:.1f}" fill="#9aa1af" font-size="9.5" '
               f'font-style="italic" font-family="Georgia,serif">starting bankroll · $1,000 each</text>')
    lines = []
    for key, name, avatar, criterion in FD_PERSONAS:
        col = FDT_SERIES_COLORS.get(key, "#9aa1af")
        mark = _persona_monogram(key, name)
...
.cmp-legend { display: flex; flex-wrap: wrap; gap: .45rem .9rem; margin-top: .9rem; padding-top: .8rem;
  border-top: 1px solid var(--fdline); }
.cmp-legend > span { display: inline-flex; align-items: center; gap: .35rem; font-family: var(--sans);
  font-size: .66rem; font-weight: 700; color: var(--fdmut); white-space: nowrap; }
/* the legend rides small — the charts, not the mascots, carry this section */
.cmp-legend .fd-agent-mascot { width: 22px; height: 22px; margin-right: 0; border-radius: 5px; }
...
/* A desk with no artwork of its own gets no reserved hole under the nameplate. */
.dk-scene:empty { display: none; margin: 0; }
